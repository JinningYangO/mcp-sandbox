...
import { ClientOptions } from 'mcp-sandbox-api';
...
export { ClientOptions } from 'mcp-sandbox-api';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'mcp_sandbox_api_api',
...
export function initMcpServer(params: {
  ...
  const client = new McpSandboxAPI(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client = params.client || new McpSandboxAPI({ defaultHeaders: { 'X-Stainless-MCP': 'true' } });
