...
const client = new McpSandboxAPI({
  apiKey: process.env['MCP_SANDBOX_API_API_KEY'], // This is the default and can be omitted
});
...
const pets: McpSandboxAPI.PetListResponse = await client.pets.list();
```
...
const pets = await client.pets.list().catch(async (err) => {
  if (err instanceof McpSandboxAPI.APIError) {
...
1. Via the `MCP_SANDBOX_API_LOG` environment variable
...
const client = new McpSandboxAPI({
  logger: logger.child({ name: 'McpSandboxAPI' }),
...
const client = new McpSandboxAPI({ fetch });
```
...
```ts
import McpSandboxAPI from 'npm:mcp-sandbox-api';
